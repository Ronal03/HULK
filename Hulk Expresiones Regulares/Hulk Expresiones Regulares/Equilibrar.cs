    ...
    internal class Equilibrar
    {
        ...
        internal static string[] OperacionesBooleanas(string input)
        {
            ...
            for (int i = 0; i < input.Length; i++)
            {
                if (input[i] == '"')
                {
                    if (i + 1 == input.Length) { throw new Exception("Faltan comillas"); }
                    ...
                    for(int j = i+5;j< input.Length;j++)
                    {
                        ...
                        { 
                            ...
                            if (count == 0)
                            {
                                ...
                                if (j + 1 < input.Length) { result[2] = input.Substring(j + 1, input.Length - j - 1); }
                                else { result[2] = ""; }
                                result[3] = "true";
                                return result;
                            }
        ...
        internal static string[] EquilibrarIf_Else(string input, string[] grupos)
        {
            ...
            for (int i = 0; i < input.Length; i++)
            {
                ...
                if (i+2<input.Length && input.Substring(i, 2) == "if" && !char.IsLetterOrDigit(input[i + 2]) && (i-1<0 || !char.IsLetterOrDigit(input[i - 1])) && bandera1) { grupos[0] = input.Substring(0, i);bandera1 = false; }
                        ...
                        if (input.Substring(j, 4) == "else" && count == 0 && input[j - 1] == ' ' && input[j + 4] == ' ')
                        {
                            ...
                            for(int k = j + 4; k < input.Length; k++)
                            {
                                ...
                                { 
                                    ...
                                    if (count2 == 0)
                                    {
                                        grupos[3] = input.Substring(j + 4,k-j-4).Trim();
                                        grupos[5]=input.Substring(k,input.Length - k).Trim();
                                        grupos[4] = "true";
                                        return grupos;
                                    }
                                 } 
                            }
                            if (count2 != 1) { throw new Exception("Los parentesis no estan balanceados"); }
        ...
        internal static string[] EquilibrarLet_In(string input, string[] grupos)
        {
            ...
            for (int i = 0; i < input.Length - 3; i++)
            {
                ...
                if (input.Substring(i, 2) == "in" && input[i + 2] == ' ' && input[i - 1] == ' ')
                {
                    ...
                    {
                        int pos2 = i;
                        grupos[0]=input.Substring(0,pos1-4);
