    ...
    internal class Aritmetica
    {
        ...
        internal static string Evaluar(string expression)
        {
            ...
            if (error != -1)
            {
                if (expression[error] == '(')
                {
                    throw new Exception( "! SYNTAX ERROR: Missing closing parenthesis");
                }
                if (expression[error] == ')')
                {
                     throw new Exception("! SYNTAX ERROR: Missing open parenthesis");
                }
                if (char.IsDigit(expression[error]) && expression[error+1]=='(') { throw new Exception("Se esperaba (*) antes del parentesis"); }
                if (char.IsDigit(expression[error]) && expression[error-1]==')') { throw new Exception("Se esperaba (*) despues del parentesis"); }
                if (expression[error] == '+' || expression[error] == '-' || expression[error] == '*' || expression[error] == '/' || expression[error] == '^' || expression[error] == '%') {return  " Operador " + expression[error]+" en posicion incorreta" ; }
                if (expression[error]=='.')
                {
                ...
                {
                    throw new Exception( "! LEXICAL ERROR: " + expression[error] + " is not valid token.");
                }
            }
        ...
        // Método que determina si el operador 2 tiene mayor o igual precedencia que el operador 1 exceptuando los parentesis abiertos
        // La precedencia es: paréntesis > potencia > multiplicación y división > suma y resta
        // El método devuelve true si el operador 2 tiene mayor o igual precedencia que el operador 1, y false en caso contrario
